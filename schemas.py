...
class AsyncRolloutRequest(BaseModel):
    ...
    def initialize_request(cls, values):
        ...
        if not (tools := values.get("tools")):
            raise ValueError("tools is required for AsyncRolloutRequest initialization")
    ...
    def _handle_apply_chat_template(
        ...
        if type(messages[0]) != dict:
            messages = [{"role":msg.role, "content":msg.content[0]['text'] if type(msg.content) == list else msg.content} for msg in messages]
        else:
            messages = [{"role":msg["role"], "content":msg["content"][0]['text'] if type(msg["content"]) == list else msg["content"]} for msg in messages]
