...
class AgentModeDaemon:
        ...
        def proxy(path):
            ...
            if current_time - last_request_time > 60 or num_requests == 1 or num_requests % 100 == 0:
                ...
                try:
                    ...
                    data_str = str(request.get_data())  # fallback
                print(f"Proxying {request.method} request to {target_server}. Request data: {data_str}")
    ...
    def get_train_data_batch(self, max_prompt_length, max_response_length, device):
        ...
        # MAX_TRANSITIONS = 32  # 进一步减少到 20 个 transitions
        # if n_transition > MAX_TRANSITIONS:
        #     import random
        #     indices = list(range(n_transition))
        #     random.shuffle(indices)
        #     selected_indices = indices[:MAX_TRANSITIONS]
            
        #     # 确保所有列表长度一致
        #     input_ids_list = [input_ids_list[i] for i in selected_indices]
        #     input_attention_mask_list = [input_attention_mask_list[i] for i in selected_indices]
        #     response_ids_list = [response_ids_list[i] for i in selected_indices]
        #     response_attention_mask_list = [response_attention_mask_list[i] for i in selected_indices]
        #     reward_list = [reward_list[i] for i in selected_indices]
        #     data_id_list = [data_id_list[i] for i in selected_indices]
        #     rollout_id_list = [rollout_id_list[i] for i in selected_indices]
        #     turn_index_list = [turn_index_list[i] for i in selected_indices]
        #     is_drop_list = [is_drop_list[i] for i in selected_indices]
