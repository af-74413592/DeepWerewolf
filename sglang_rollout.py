...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        # self._engine.tokenizer_manager.tokenizer = self.processing_class
        try:
            # This is when processing_class is a tokenizer
    ...
    async def chat_completion(self, json_request):
        ...
        output["input_ids"] = req.input_ids.tolist()[0]
        # it can be Dict or AsyncIterator[Dict]
        if isinstance(output, dict):
            outputs = [output]
        else:
            outputs = output
        request = ChatCompletionRequest(**json_request)
        response = self.openai_serving_chat._build_chat_response(request,outputs,int(time.time()))
        print(response)
        return response
    ...
    # async def chat_completion(self, request, rawrequest):
    #     return await self.openai_serving_chat.handle_request(
    #     request, raw_request
    # )
